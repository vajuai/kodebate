 ...
 * @param rounds 辩论轮次
 */
data class DebateParameters(val topic: String, val rounds: Int)
/**
 * 创建辩论主持人代理
 */
fun createDebateHostAgent(
    ...
    val strategy = strategy<String, String>("debate-session", toolSelectionStrategy = ToolSelectionStrategy.NONE) {
        ...
        val conDebateSession by subgraphWithTask<String>(
            tools = scriptToolSet.asTools() + listOf(SayToUser),
            llmModel = GoogleModels.Gemini1_5Pro  // 改用更稳定的模型
        ) { userInput ->
            """
        ...
        val decreaseRoundNode by node<String, String> { input ->
            theRound-- // 这行代码现在在一个每次都会被执行的节点内部了！
            println("✅ 轮次已递减, 剩余轮次: $theRound")
            input // 将数据原样传递下去
        }
        ...
        */
        ...
        edge(loadMemory forwardTo proDebateSession)
        edge(proDebateSession forwardTo retrieveProResult)
        edge(retrieveProResult forwardTo saveProToMemory)
        edge(saveProToMemory forwardTo conDebateSession)
        edge(conDebateSession forwardTo retrieveConResult)
        edge(retrieveConResult forwardTo saveConToMemory)
        edge(saveConToMemory forwardTo decreaseRoundNode)
        edge((decreaseRoundNode forwardTo proDebateSession) onCondition { theRound >= 1 })
        edge((decreaseRoundNode forwardTo conSummary) onCondition { theRound < 1 })
...
open class DebaterToolSet : ToolSet {

    @Tool
    @LLMDescription("分析辩论策略")
    open fun analyzeDebaterStrategy(
...
open class OpinionToolSet : ToolSet {

    @Tool
    @LLMDescription("分析和总结辩论论点")
    open fun analyzeDebateOpinions(
...
open class ScriptToolSet : ToolSet {

    @Tool
    @LLMDescription("记录辩论发言。辩手必须使用此工具记录自己的发言内容。")
    open fun recordDebateScript(
...
/**
 * 解析用户输入，提取辩论主题和轮次
 * 格式: "辩论主题 / 轮次" (例如: "人工智能是否会取代人类 / 5")
 * 如果不提供轮次，则使用默认值。
 * @param userInput 用户输入的完整字符串
 * @param defaultRounds 默认轮次
 * @return DebateParameters 包含主题和轮次的数据对象
 */
fun parseDebateInput(userInput: String, defaultRounds: Int = 3): DebateParameters {
    val parts = userInput.split("/").map { it.trim() }
    val topic = parts[0]
    val rounds = if (parts.size > 1) {
        parts[1].toIntOrNull() ?: defaultRounds
    } else {
        defaultRounds
    }
    return DebateParameters(topic, rounds)
}
...
fun main(): kotlin.Unit = runBlocking {
    ...
    // 1. 获取用户输入
    println("请输入辩论主题和轮次 (格式: <主题> / <轮次>)，轮次为可选参数。")
    println("例如: 人工智能是否会取代人类 / 5")
    print("➡️ ")
    ...
    // 2. 解析输入，如果用户不指定轮次，则使用默认值 3
    val debateParams = parseDebateInput(userInput, defaultRounds = 3)
    ...
    val agent = createDebateHostAgent(
        ...
        hostToolSet = DebateHostToolSet(),
        memoryProvider = memoryProvider,
        featureName = "debate-host",
        productName = "debate-system",
        organizationName = "ai-debate",
        maxAgentIterations = 250,
        defaultRound = debateParams.rounds // 关键点：将解析出的轮次传递给 Agent
    )

    // 开始辩论
    println("🎭 AI vs AI 辩论开始...")
    println("   主题: ${debateParams.topic}")
    println("   轮次: ${debateParams.rounds}")
    try {
        // 关键点：将解析出的主题作为输入运行 Agent
        val result = agent.run(debateParams.topic)
        println("\n🎉 辩论最终结果：\n$result")
    } catch (e: Exception) {
        when {
...
open class DebateHostToolSet : ToolSet {

    @Tool
    @LLMDescription("评判辩论获胜方并给出详细评分")
    open fun judgeDebateWinner(
    ...
    open fun analyzeDebateHighlights(
